...
import { createDeal } from "../store/dealSlice";
import { useNavigate } from "react-router-dom";
import toast from "react-hot-toast";
import ImageUpload from "./ImageUpload"; // Reuse ImageUpload component
...
const AddDeal = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    deal_price: "",
    deal_stock: "",
    deal_images: [], // Store Cloudinary URLs
    category: "",
    deal_code: "",
    deal_expiry: "",
    bg_color: "#FFFFFF",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await dispatch(createDeal(dealData)).unwrap();
      console.log("AddDeal - Success:", result);
  ...
  return (
    <div className="p-6 bg-white rounded-xl shadow-lg max-w-2xl mx-auto">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Add New Deal</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        >
          {loading || imageUploading ? "Creating..." : "Create Deal"}
        </button>
      </form>
    </div>
  );
